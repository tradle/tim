    ...
    const logProcessor = createProcessor({
      ...
      worker: co(function* (data, cb) {
        ...
        try {
          ...
          // debugger
          debug('failed to process entry', data)
        } finally {
          cb()
        }
      })
  ...
  async newObject (msg) {
    ...
    if (payload[TYPE] === MESSAGE) {
      ...
      try {
        ...
        let bot = this._getItem(PROFILE + '_' + obj.from[ROOT_HASH])
...
// console.log(pubKeys)
      ...
      for (var [verification, author] of sourceToAuthor) {
        let a = this._getItem(PROFILE + '_' + author.permalink)
      ...
      payload.providerInfo = utils.clone(this._getItem(PROFILE + '_' + msg.objectinfo.author).organization)
      // debugger
    }
    ...
    let toId = [PROFILE, msg.recipient].join('_')
    ...
    if (rtype === PRODUCT_APPLICATION  &&  me.isEmployee) {
      let bot = this._getItem(PROFILE + '_' + old.from[ROOT_HASH])
  ...
  filterChatMessages(messages, orgId, lastId) {
    ...
    for (let i=messages.length - 1; i>=0; i--) {
      ...
      if (r[TYPE] === FORM_REQUEST  &&  !r.document) {// && r._documentCreated)
      // delete list[id]
...
    } catch (err) {
  ...
  addMessagesToChat(id, r, isInit, timeShared) {
    ...
    if (r[TYPE] === SELF_INTRODUCTION)
      return
    if (r[TYPE] === SIMPLE_MESSAGE  &&  r.message === '[already published](tradle.Identity)')
      return
    if (r._context) {
      let c = this._getItem(r._context)
      // context could be empty if ForgetMe was requested for the provider where form was originally created
  ...
  async onAddVerification(params) {
    ...
    try {
      ...
      if (!isReadOnly) {
        let vr = this._getItem(r.document)
      ...
      await this.dbPut(verificationRequestId, verificationRequest);
    }
    catch(err) {
      debugger
      err = err
  ...
  async onGetItem(key, action, noTrigger) {
    // await this._loadedResourcesDefer.promise
    let r = this._getItem(utils.getId(key))
    var resource = {};
    if (utils.isMessage(this.getModel(r[TYPE]))) {
      let res = await meDriver.objects.get(r[CUR_HASH])
      extend(resource, res.object)
    }
  ...
  async onAddItem(params) {
    ...
    if (isGuestSessionProof || isBecomingEmployee) {
      ...
      if (!status.watches.link  &&  !status.link) {
        ...
        await self.publishMyIdentity(rep, params.disableAutoResponse)
      }
      else {
        let orgRep
        if (isBecomingEmployee) {
        ...
        await self.onAddMessage({msg: msg, disableAutoResponse: params.disableAutoResponse})
      }
    }
    let results = []
    ...
    if (isRegistration)
      ...
      await handleMessage(params.noTrigger, returnVal)
    else
  ...
  async getList(params) {
    ...
    var {modelName, prop, list, to, context, loadEarlierMessages, allLoaded, fromView, isAggregation, isForgetting, sponsorName, isTest, start, limit, listView, _readOnly, spinner} = params
    var isMessage = utils.isMessage(meta)
    if (!isMessage) {
      fromView = true
      ...
      if (!result) {
        ...
        //   this.trigger({action: 'list', alert: translate('notFound', meta)})
        return
      }
      if (!SERVICE_PROVIDERS)
      ...
      retParams.gridView = params.gridView
...
//       }
    ...
    if (isBacklinkProp) {
      for (let i=j; i<thisChatMessages.length; i++) {
        addLink(modelName, links, thisChatMessages[i])
      ...
      for (let i=j; i>=0; i--) {
        let l = addLink(modelName, links, thisChatMessages[i])
    ...
    if (!links.length)
      return
    return Promise.all(links.map(r => meDriver.objects.get(r)))
    .then((result) => {
      list = transformResult(result)
      if (isBacklinkProp) {
    ...
    function transformResult(result) {
      return result.map((msgInfo) => {
        ...
        let cachedRes = self._getItem(utils.getId(r))
        extend(r, cachedRes)
        return r
      })
  ...
  monitorTim() {
    this._keeper = promisify(meDriver.keeper)
    this.monitorLog()
    var self = this
    // meDriver.ready()
    // .then(function() {
...
//       })
  ...
  async putMessageInDB(val, obj, batch, onMessage) {
    ...
    if (obj.objectinfo && obj.objectinfo.author)
      fromProfile += obj.objectinfo.author
    else
      fromProfile += obj.from[ROOT_HASH]
