...
// debugger
    ...
    // this.driver = new Driver(this, driverInfo, this._enginePromise, this._resolveWithEngine, db, this._loadedResourcesDefer, SERVICE_PROVIDERS)
...
if (!res[SIG]  &&  res._message)
  ...
  onGetDetails(resource) {
    this.trigger({action: 'showDetails', resource})
  },
  onGetDocuments(resource, docs) {
    let list = docs.map((r) => this._getItem(utils.getId(r)))
    this.trigger({action: 'showDocuments', list, resource})
  },
  getItem(resource) {
...
debugger
  ...
  async shareAll(resources, to, formResource) {
    ...
    this.trigger({action: 'addItem', context: formResource.context, resource})
    if (documentCreated)
      return
  ...
  async getShareableResources(params) {
    ...
    var productsToShare = await this.searchSharables({modelName: MY_PRODUCT, to: utils.getMe(), strict: true, limit })
  ...
  async getDriver(me) {
    ...
    // let { ret, driver, multiqueue, queueReceive } = await this.driver.getDriver(me)
      ...
      items.forEach((r, i) => {
        ...
        if (r._author !== fromHash) {
          if (!allMyIdentities.includes(r._author)) {
            // Probably from employee that is analyst
            debugger
            // return
          }
          else
