...
var noopFnPath = path.join(__dirname, "./no-op.js");
...
if (NODE_ENV === "development") {
  ...
  config = merge(common, {
    ...
    module: {
      ...
      loaders: [getBabelLoader()].concat(getMaybeLoaders())
    },
    plugins: [
      // new webpack.optimize.DedupePlugin(),
...
function getBabelLoader() {
  const loader = {
    ...
    //   return false
    // },
    exclude: [/errno/, /xtend/, /react-dropzone/],
    query: {
      compact: false,
      cacheDirectory: true,
  ...
  if (!isProd && screwIE) {
    ...
    loader.exclude.push(
      /node_modules\/(?!react|tcomb|rn-markdown|\@tradle\/aws-client|\@tradle\/ethereum-adapter-etherscan|\@tradle\/promise-utils|\@tradle\/embed|\@tradle\/engine|node-irr)/ );
  }
