...
class ReviewPrefilledItemsList extends Component {
  constructor(props) {
    super(props);
    ...
    this.state = {
      list: list || [],
      products: [],
      resource: this.props.resource.prefill,
      dataSource: list &&  dataSource.cloneWithRows(list) || dataSource,
    }
  ...
  renderRow(resource, sectionId, rowId)  {
    ...
    let isNewForm = false //!resource[ROOT_HASH]  &&  !resource._sourceId
  ...
  async selectResource(resource, rowId) {
    ...
    if (model.notEditable  &&  resource[ROOT_HASH]) {
      resource.from = this.props.resource.to
      resource.to = this.props.resource.from
...
reactMixin(ReviewPrefilledItemsList.prototype, Reflux.ListenerMixin);
...
reactMixin(ReviewPrefilledItemsList.prototype, OnePropFormMixin);

module.exports = ReviewPrefilledItemsList;
