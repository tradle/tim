...
const RCTAsyncStorage = NativeModules.AsyncRocksDBStorage ||
  NativeModules.AsyncSnappyStorage ||
...
var AsyncStorage = {
  ...
  getAllKeysInRange: function(
    lte: string,
    gte: string,
    callback?: ?(error: ?Error, keys: ?Array<string>) => void
  ): Promise {
    return new Promise((resolve, reject) => {
      const finish = (err, keys) => {
      ...
      if (RCTAsyncStorage.getAllKeysInRange) {
        return RCTAsyncStorage.getAllKeysInRange(lte, gte, finish);
      }

      AsyncStorage.getAllKeys((err, keys) => {
        if (keys) {
          keys = keys.filter(key => key <= lte && key >= gte);
        }
  ...
  getAllKeysWithPrefix: function(prefix: string, callback?: ?(error: ?Error, keys: ?Array<string>) => void): Promise {
    return new Promise((resolve, reject) => {
      const finish = (err, keys) => {
        err = convertError(err)
        if (callback) callback(err, keys);
      ...
      if (RCTAsyncStorage.getAllKeysWithPrefix) {
        return RCTAsyncStorage.getAllKeysWithPrefix(prefix, finish);
      }
